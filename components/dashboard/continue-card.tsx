 ...
 */
export function ContinueCard({ complaint, showViewAllDrafts }: ContinueCardProps) {
  return (
      ...
      <div className="bg-primary text-primary-foreground shadow-lift flex flex-col gap-5 rounded-2xl p-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-start gap-4">
          {/*
            ...
            from the green.
          */}
          <GovernmentLogo iconKey={complaint.entityIconKey} className="border-transparent" />
          <div className="flex min-w-0 flex-col gap-1.5">
            <h3 className="text-base font-semibold text-balance">{complaint.title}</h3>
            <p className="text-primary-foreground/70 text-sm">{complaint.entityName}</p>
            <div className="text-primary-foreground/70 mt-1 flex flex-wrap items-center gap-3 text-xs">
              <span className="inline-flex items-center gap-1.5">
                <Clock className="h-3 w-3" aria-hidden="true" />
                آخر تعديل <RelativeTime iso={complaint.updatedAt} />
              </span>
